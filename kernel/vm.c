...
int u2kvmcopy(pagetable_t u_pagetable,pagetable_t k_pagetable, uint64 k_oldsize,uint64 k_newsize)
{
  ...
  k_oldsize=PGROUNDUP(k_oldsize);
  for(uint64 i=k_oldsize;i<k_newsize;i+=PGSIZE)
  {
    /* 拿到指定虚拟地址对应用户页表的pte */
    if((pte_from=walk(u_pagetable,i,0))==0)
      panic("u2kvmcopy: src pte does not exist");;
    /* 拿到(创建)指定虚拟地址对应内核页表的pte */
    if((pte_to=walk(k_pagetable,i,1))==0)
      panic("u2kvmcopy: dst pte walk faild");
    /* 赋值并更改权限 */
